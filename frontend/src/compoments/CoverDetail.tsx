...
import type { MixedSelector } from "./SelectCoverDialog";
...
type DialogProps = {
  ...
  reserve: string;
  setReserve: React.Dispatch<React.SetStateAction<string>>;
}

function CoverDetail({list, page, setPage, setOpen, reserve, setReserve}: DialogProps){
  ...
  return (
    <div className="bg-black inset-0 fixed flex flex-col z-150 pb-8 sm:bg-black/80 sm:pb-25">
      ...
      {list.map((p, index)=>(
        <div key={index} onClick={()=>setPage(index)} className={`h-100 flex justify-center relative items-center lg:h-100 ${
          page === index
            ? "block"
            : "hidden"
        }`}>
          <img src={p.url} alt="photo" className="w-full h-full object-contain" />
            <input type="radio" value={p.url} name="Coverpage" checked={reserve === p.url} className="absolute right-5 top-4 w-8 h-8 sm:right-60 sm:top-4" onChange={(event)=>setReserve(event.target.value)} />
        </div>
      ))}

      <div className="w-full flex justify-center mt-auto">
        <div className="flex gap-2 h-20 overflow-x-auto items-center sm:w-150 sm:h-22 scrollbar-none justify-center">
          {list.map((p, index)=>(
        ...
        ?
          <div className="flex w-full mt-4 justify-center gap-7">
            <button className="text-white border border-white px-4 py-1 text-xs rounded-2xl flex items-center gap-1 hover:bg-gray-800 active:bg-gray-500 sm:py-2">
              ...
              <span className="sm:hidden">Web Quality</span>
              <span className="hidden sm:block">Web Quality Download</span>
            </button>
            ...
            <button className="text-white border border-white px-4 py-1 text-xs rounded-2xl flex items-center gap-1 hover:bg-gray-800 active:bg-gray-500 sm:py-2">
              ...
              <span className="sm:hidden">Print Quality</span>
              <span className="hidden sm:block">Print Quality Download</span>
            </button>
          </div>
        :
          <div className="flex w-full mt-4 justify-center gap-7">
