...
type DialogProps = {
  setSelectp: React.Dispatch<React.SetStateAction<boolean>>;
  ophoto: Selector[];
  setOphoto: React.Dispatch<React.SetStateAction<Selector[]>>;
  setSelectedPhoto: React.Dispatch<React.SetStateAction<string[]>>;
}


function SelectCoverDialog({setSelectp, ophoto, setOphoto, setSelectedPhoto}:DialogProps){
  const [open, setOpen] = useState(false);
  const [page, setPage] = useState(0);

  const picker = (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
    const newOphoto = ophoto.map((p, i) => (
      i === index
        ?
  ...
  const finalPicker = () => {
    const newSelectedPhoto = ophoto.filter(p =>(
      p.isSelected === true
    )).map(p => p.url);
  ...
  const selectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    var newSelectedPhoto = [...ophoto];

    if (checked === false){
      newSelectedPhoto = ophoto.map(p => (
        {url: p.url, isSelected: false}
      ));
    } else {
      newSelectedPhoto = ophoto.map(p => (
        {url: p.url, isSelected: true}
      ));
    }

    setOphoto(newSelectedPhoto);
  }


  return (
    <div className="flex inset-0 fixed bg-black/40 justify-center items-center z-100">
      {open && <PhotoDetail list={ophoto} page={page} setPage={setPage} setOpen={setOpen} picker={picker} />}
      <div className="flex flex-col bg-white w-[95%] rounded-2xl mt-13 h-[80vh] px-8 pb-5 pt-14 overflow-auto relative sm:w-[55%] sm:h-[85%]">
        <div className="flex flex-col gap-1">
          <span className="text-xl font-bold">Select Display Photos</span>
          <span className="text-gray-500 text-[10px] sm:text-xs">Please select photos to be displayed on the first page.</span>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" onClick={()=>setSelectp(false)} className="size-6 text-gray-400 rounded absolute top-4 right-6  hover:bg-gray-100 active:bg-gray-200">
        ...
        <div className="grid grid-cols-3 content-start px-1 gap-3 justify-between overflow-auto h-[75%] sm:grid-cols-5 sm:gap-3 sm:px-3 sm:pt-6">
          {ophoto.map((p, index) => (
            <div key={index} className="aspect-square sm:px-3 sm:py-3 sm:hover:bg-gray-200 relative rounded-lg">
              <input type="checkbox" className="absolute w-4 h-4 right-1 top-1 sm:right-4 sm:top-4 z-50" checked={p.isSelected === true} onChange={(event)=>{picker(index, event)}} />
        ...
        <div className="flex items-center gap-2 px-2 sm:hidden">
          <input type="checkbox" checked={ophoto.every(p => p.isSelected === true)} onChange={(event)=>selectAll(event)} className="w-4 h-4" />
          <label className="text-sm">Select All</label>

          <div className="bg-gray-200 px-3 flex items-center rounded-lg py-1 gap-1.5 ml-auto">
            <span className="font-bold text-sm">{`${ophoto.filter(p=> p.isSelected === true).length} / ${ophoto.length}`}</span>
        ...
        <div className="flex gap-2 w-full mt-4 sm:mt-0 sm:px-4">
          ...
          <button onClick={()=>setSelectp(false)} className="border-2 border-black ml-auto flex justify-center w-24 py-1 rounded-4xl font-bold hover:bg-gray-200 active:bg-gray-300">
            Cancel
          </button>
