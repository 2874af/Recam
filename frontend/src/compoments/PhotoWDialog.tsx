...
type PhotoWDialogProps = {
  pw: File[];
  setPw: React.Dispatch<React.SetStateAction<File[]>>;
  setPwsuccess: React.Dispatch<React.SetStateAction<boolean>>;
  setPwopen: React.Dispatch<React.SetStateAction<boolean>>;
}

function PhotoWDialog({pw, setPw, setPwsuccess, setPwopen}:PhotoWDialogProps){

  const InputRef = useRef<HTMLInputElement>(null);
  const [preview, setPreview] = useState<File[]>(pw);
  ...
  return (
    <div className="inset-0 fixed bg-black/40 flex justify-center items-center">
      <div className="flex flex-col bg-white w-230 rounded-2xl px-13 pt-6 pb-7">
        <div className="flex">
          <span className="text-2xl font-bold">Upload Photography-W</span>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" onClick={()=>setPwopen(false)} className="size-6 text-gray-400 rounded ml-auto  hover:bg-gray-100 active:bg-gray-200">
