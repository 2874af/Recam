...
function PhotoDetail({list, page, setPage, setOpen, picker}: DialogProps){
    ...
    <div className="bg-black inset-0 fixed flex flex-col z-150 pb-8 sm:bg-black/80 sm:pb-25">
      ...
      {list.map((p, index)=>(
        <div key={index} onClick={()=>setPage(index)} className={`h-100 flex justify-center relative items-center ${
          ...
          <input type="checkbox" checked={p.isSelected === true} className="absolute right-5 top-4 w-8 h-8 sm:right-60 sm:top-4" onChange={(event)=>{picker(index, event)}} />
        </div>
      ))}
