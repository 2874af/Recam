...
function FloorDetail({list, page, setPage, setOpen, picker}: DialogProps){
    ...
    <div className="bg-black inset-0 fixed flex flex-col z-150 pb-8 sm:bg-black/80 sm:pb-25">
      ...
      {list.map((f, index)=>(
        <div key={index} onClick={()=>setPage(index)} className={`h-100 flex justify-center relative items-center ${
          ...
          <img src={f.url} alt="photo" className="w-full h-full object-contain" />
          <input type="checkbox" checked={f.isSelected === true} className="absolute right-5 top-4 w-8 h-8 sm:right-60 sm:top-4" onChange={(event)=>{picker(index, event)}} />
        </div>
      ))}
        ...
        <div className="flex gap-2 h-20 overflow-x-auto items-center sm:w-150 sm:h-22 scrollbar-none justify-center">
          {list.map((f, index)=>(
            <div key={index} onClick={()=>setPage(index)} className={`relative shrink-0 ${
              ...
              <img src={f.url} alt="photo" className="w-full h-full object-cover" />
